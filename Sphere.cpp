...
void Sphere::init(int prec) {
	...
	for (int i = 0; i <= prec; i++) {
		for (int j = 0; j <= prec; j++) {
			float tianding = 180.0f - i * 180.0f / prec;
			float y = (float)cos(toRadians(180.0f - i * 180.0f / prec));
			float thela = j*360.0f / prec; 
			float  ab = abs(cos(asin(y)));
			float x = -(float)cos(toRadians(j*360.0f / prec))*(float)abs(cos(asin(y)));
			float z = (float)sin(toRadians(j*360.0f / (float)(prec)))*(float)abs(cos(asin(y)));
			std::ostringstream oss;
			oss << "tianding = " << tianding << ", fangwei = " << thela << ", ab = " << ab <<  ", y = " << y << ", x = " << x << ", z = " << z;

			std::string line = oss.str();

             
            Utils::createTxtFile(line);
